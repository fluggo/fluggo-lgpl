...
namespace Fluggo.CodeGeneration.IL {
	/// <summary>
	/// Represents an expression for a constant 32-bit unsigned integer.
	/// </summary>
	[CLSCompliant( false )]
	public sealed class UInt32ConstantExpression : Expression
	{
		uint _value;

		/// <summary>
		/// Creates a new instance of the <see cref="UInt32ConstantExpression"/> class.
		/// </summary>
		/// <param name="value">Value of the integer.</param>
		public UInt32ConstantExpression( uint value ) {
			_value = value;
		}
		...
		///   This property always returns <see cref="UInt32"/>.</value>
		...
		public override string ToString() {
			return _value.ToString( CultureInfo.InvariantCulture ) + "U";
		}
			...
			switch( _value ) {
				...
				default:
					if( _value <= 127 ) {
						...
						cxt.Generator.Emit( OpCodes.Ldc_I4_S, unchecked((byte)(sbyte) _value) );
					}
					else {
						cxt.Generator.Emit( OpCodes.Ldc_I4, unchecked((uint) _value) );
					}
