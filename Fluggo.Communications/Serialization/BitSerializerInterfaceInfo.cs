...
namespace Fluggo.Communications.Serialization
{
	class BitSerializerMethodInfo {
		...
		public BitSerializerMethodInfo( MethodInfo method, BitSerializerOptions options ) {
			if( method == null )
				throw new ArgumentNullException( "method" );

			_method = method;
			_attributes = method.GetCustomAttributes( true );
		}
	}
	
	/// <summary>
	/// Interprets the serialization options represented by a list of attributes on an interface.
	/// </summary>
	class BitSerializerInterfaceInfo
	{
		BitSerializerMethodInfo[] _methods;
		object[] _attributes;
		static readonly BitSerializerOptions __defaultOptions = new BitSerializerOptions();
		BitSerializerOptions _options;
		int _maxLength = -1;

		public BitSerializerInterfaceInfo( Type interfaceType, BitSerializerOptions options ) {
			if( paramType == null )
				throw new ArgumentNullException( "fieldType" );

			_paramName = paramName;
			_attributes = interfaceType.GetCustomAttributes( true );
			_options = options;
			...
			List<object> dynElemAttrs = new List<object>();
			_maxTypeCode = -1;

			foreach( object attr in _attributes ) {
				DerivedTypeCodeAttribute codeAttr = attr as DerivedTypeCodeAttribute;
				ArrayElementAttribute elemAttr = attr as ArrayElementAttribute;
				...
				else if( elemAttr != null ) {
					dynElemAttrs.Add( elemAttr.Attribute );
				}
