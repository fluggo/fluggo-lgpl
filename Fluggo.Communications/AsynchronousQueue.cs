...
namespace Fluggo.Communications
{
	...
	public sealed class AsynchronousQueue<T> : IDisposable {
		class WaitQueueRequest : BaseAsyncResult {
			...
			public void Complete( T result, bool raiseEventSynchronously ) {
				_result = result;
				Complete( _willCompleteAsync, raiseEventSynchronously );
			}
		...
		/// <summary>
		/// Creates a new instance of the <see cref='AsynchronousQueue'/> class.
		/// </summary>
		...
		public void Enqueue( T value ) {
			...
			// Satisfy asynchronous queue (on another thread, if need be)
